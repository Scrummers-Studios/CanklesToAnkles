...
### Animations
The animation system simply consists of 4 animations, Running which serves as the Initial state between actions. From the running animation we have the two actions Jumping and Rolling which are the two actions the player can take. All animations can be interrupted with the exception of Incoming transitions to Jumping, Rolling and Landing. This is done to make the player responsive in fast paced scenarios. The way this is implemented is that both the incoming and outgoing transitions "Running" and the outgoing transition of "Landing" have their "Interruption source: Next state" meaning they can be interrupted by either Rolling or Jumping. The conditions for the transitions are based on the different player states determined by the physics of the player. To make sure these are updated frequently enough the fixed intervals are calculated 100 times each second meaning any framerate equal to or below will be in sync with the player animations. 
...
### HealthScript
The health script is orchestrating the whole mechanics for the health. It is built up with some basic methods that can alter the health of the player, in terms of being damaged or healed. If one of these methods alter the health to the maximum, the player will not achieve any more healing. Otherwise if the player's health is altered below the minimum requirement, the player will respawn at the start of the stage. When the player respawns all of the pickups, enemies and the environment will be sent back to the original position at level start.
<p align="center">
  <img src="/docs/imgs/healthScript.png"/>
</p>
As seen in the picture above, the script needs the environment, player and the picture in the health bar. The script also allows some basic settings such as the default damage taken, the current health as well as the maximum health and the default health. The bar in the health script gets updated every time the health of the player is altered, this is done by setting the fill property of the image to the current health divided by the maximum health. Our health bar is somewhat different than other health bars, it is inverted, representing *diabetes*. So the full equation for the bar is actually 1 - currentHealth/ maximumHealth. Respawn is a method that is public in the healthScript, this is because the camera needs the ability to both deal damage to the player and respawn the player.

Initially we thought it would be best to implement both health and lives, however after further discussion we decided to only use health. The first iteration of the health script consisted of both health and lives, with the ability to turn on or off the usage of the lives. We were planning on using lives as the health bar depleted, and after all the lives had been used, the player would need to restart the game at level 1. The first iteration of the health was text in the canvas that displayed both the lives and the health of the player, later it was exchanged for just the health and as a final draft a fully functional health bar.

### CameraScript
The camera needs to be able to detect if the player is slowing down and ends up outside of the view to the camera. This is done by using frustum planes. By creating many planes for the view angle of the camera and iterating through them, the camera detects if the player's boxCollider is inside or outside these planes. This is a continuous check that goes every frame. If the player happens to be outside of these planes, the camera script will respawn the character.
...
Second problem we encountered was that the player had the ability to respawn inside objects. By respawning the character in the center of the screen, the terrain might have gained some altitude or an obstacle might be in the way. This caused the player to respawn inside these objects and can't move. This was solved by resetting the position of the environment and all the components of the game. The one thing we did not set back to its original position is the camera, this could have been implemented, but we decided not to do this cause the camera will jump back to its place and it looked laggy.

### Pickups and enemies
The pickups use a script, handle all the logic for the pickups and the enemies. Originally this script was just an enemy script that dealt some damage to the player, after some iterations we had implemented so that the enemies disappeared when colliding with the player. Some iterations later, the script was altered to a pickup script, this script allowed for changing from damage to healing of the player. With this we had the ability to implement healthy food which lowers the diabetes bar. In the last iteration some basic rotation was added to give a more intriguing impression of the pickups and the enemies.

With the script disabling the pickups after the collision the pickups will still be inactive after the player respawns, this allows the player to continue trying to clear the level if they should die from too much unhealthy food.
...
For simplicity the player's *Collider* consists of a single *BoxCollider* to detect collisions. As the gameplay consists of only linear movement across a single plane, any form of collider would suffice for the purposes of this game. As for the mechanics associated with the collider it is responsible for the Roll mechanic. The Roll mechanic simply explained is just dividing the collider in half and moving it to the initial position, and vice versa for the reversal of this process. This makes it so that the level can be designed with this in mind to give the player the possibility of shrinking its size to evade obstacles.  
...
### Goal
Each level has a goal, this goal is placed at the end of each level. The goal is made up by 2 components, the gameobject which the player comes in contact with, and the effects system. The effect system is created through unity and is a resemblance of confetti. After the player reaches the goal the confetti gets activated and starts to rain down in the scene. With the confetti raining down behind the player can also see a UI element, giving positive feedback to the player on completing the level. In the UI element you can find two buttons with the option to either proceed to the next level or quit to the main menu. Shown in the picture below the UI for completion of a level matches the rest of the theme of the UIs in the game. The confetti is also using the same color theme.

<p align="center">
  <img src="/docs/imgs/GoalScreen.png"/>
</p>
When the player is traversing through the levels, the goal will stay invisible. The gameobject goal is a 3D cube which the player can come in contact with. However this gameobject has a mesh that makes this goal visible, the goal script disables this mesh as soon as the scene starts running. This allows us to have a cleaner finish line, with the opportunity to add visuals to the goal. Since the controller disables the mesh after the scene has started, as developers, we can see the box clearly and have the ability to move it, scale it, etc. The goal script is also the script which turns on the confetti and displays the UI screen. Below is a picture of how the goal looks in the editor window, when the mesh is still enabled.
...
The user interface layout attempts to appeal to the familiarity of the player. The layout itself consists of a side menu with open space for showcasing the game state or artwork for the menu. At this stage the amount of information that needs to be displayed is minimal giving a lot of freedom when it comes to design. Given this I opted for a basic side menu to see the character and game when paused and for the menu a centered controls and settings page. 
...
At the end, the whole project was completed and we were able to get peoples final thoughts on the game. People gave us good responses on all the UIs, and thought that adding the confetti to the win screen was a fun bonus. They also gave a thumbs up on the videos being played when starting the game and between level 3 and 4. They thought it gave the character more of a back story and gave the story something extra. When it comes to the gameplay, they thought it was both fun and challenging. Some wanted there to be more levels, but due to the deadline coming up we were not able to implement any more levels.

Throughout the project having testers have been really helpful. They gave us insight that helped improve the project and made it easier to understand what was working and what didn’t.
...
###### **Petter Edvardsen**
In the process of developing this game I did features such as the Player movement and mechanics relating to the physics and object interactions, PlayerController and its relations as well as the bug fixing. I also worked on the Animations with the underlying logic. For the level design I created the Forest level as a whole with the exception of one obstacle prefab. Additionally the 1st iteration of the user interface with the layout that remained and the implementation of the logic surrounding the scene management. For the visuals I worked on the cameras with the Post-processing which was simple and straightforward. As a whole the process of researching and creating the gameplay logic, level design, animations and solving bugs has been a very time consuming but educational process.
...
To enhance the visual appeal and maintain consistency with the game's polygonal style, I updated the skybox, ensuring that it aesthetically fit with the rest of the game.    
...
###### **Sivert Ertshus**
During the development of this game I created the characters and set up character rigs along with parts of the animations and sounds. The creation of the characters happened in another program Blender, which meant that I had to learn how to use it.  I also created the design for the different user interfaces and made them fit the theme of the game, along with adding sounds to the buttons. To tell a more detailed story of the game, I created the two story videos that get played in the game. After starting the game from the main menu and between level 3 and 4 a video is played to make it a stronger story. I’ve also worked on scripts for the videos and the characters. For the video I added a button which only activates when the video is done, and for the character I made changes to isGrounded etc. and the sound the player makes when jumping and rolling.    

###### **Jakob Finstad**
Under the development process I had to focus on the camera, health and pickups. I created and altered the functions for the camera out of bounds detection, and all the functions in the pickup and health script. I've also done the implementation of the enemies, by finding the right assets to use, scale it to the right size, balance the damage/heal amount etc. At the end of each level there is a goal that I implemented as well as an event system that represents the confetti raining after a level is completed and the InGame UI. Furthermore, I created the health bar, with some inspiration and some usage of some assets. I made an image to better fit our need for a progressbar, and made the scripts handling the progression of the bar.

## Manual
The game "The Weight Escape" is built for WebGL, Mac, Linux and Windows.
When starting the game, the player sees the main menu. Here the player has a total of four choices. *Play game*, which brings the player right into the story of the game. Settings, which lets the player change the volume in the game. Controls, which opens up an image where the player sees the controls. Quit game, which stops the game from running.
When playing the game the character runs on its own, but the player needs to jump and roll to dodge obstacles. To make the character jump, the player presses space to jump and ctrl to make it rolle, and if the player ever needs to see the controls again, they can press the esc button on the keyboard, and one will be able to see the same settings as they did on the main menu. The differences are that instead of playing a game, it says resume game, and instead of quit game, it brings one back to the main menu. 
