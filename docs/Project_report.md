...
###### **Petter Edvardsen**
In the process of developing this game I did features such as the Player movement and mechanics relating to the physics and object interactions, *PlayerController* and its relations aswell as the bugfixing. I also worked on the 1st and 3rd iteration of the Animations with the underlying logic. For the level design I created the Forest level as a whole with the exception of one obstacle prefab. Additionally the 1st iteration of the user interface with the layout that remained and the implementation of the logic surrounding the scene management. For the visuals I worked on the cameras with the Post-processing which was simple and straightforward. As a whole the process of researching and creating the gameplay logic, level design, animations and solving bugs has been a very time consuming but educational process.
...
###### **Jakob Finstad**
Under the development process i had focus on the camera, health and pickups. I created and altered the functions for the camera out of bounds detection, and all the functions in the pickup and health script. I've also did the implementation of the enemies, by finding the right assets to use, scale it to the right size, balance the damage/heal amount etc. At the end of each level there is a goal that I implemented, as well as an made event system that represent the confetti raining after a level is completed and the InGame UI. Furthermore, I created the health bar, with some inspiration and some usage of some assets. I made an image to better fit our need for a progressbar, and made the scripts handling the progression of the bar.
