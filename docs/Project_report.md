...
###### **Petter Edvardsen**
In the process of developing this game I worked on aspects such as the Player movement and mechanics relating to the physics and object interactions, *PlayerController* and its relations. I also worked on the 1st and final iteration of the Animations aswell as the underlying logic as it is directly related to the physics. For the level design I created the Forest level as a whole. Additionally the 1st iteration of the user interface with the layout that remained and the implementation of the logic surrounding the scene management. Finally I worked on the cameras in realtion to the Post-processing for the visuals.
