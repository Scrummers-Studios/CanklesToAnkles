...
###### **Petter Edvardsen**
In the process of developing this game I did features such as the Player movement and mechanics relating to the physics and object interactions, *PlayerController* and its relations aswell as the bugfixing. I also created the 1st and 3rd iteration of the Animations aswell as the underlying logic. For the level design I created the Forest level as a whole with the exception of one obstacle prefab. Additionally the 1st iteration of the user interface with the layout that remained and the implementation of the logic surrounding the scene management. For the visuals I worked on the cameras with the Post-processing which was simple and straightforward. As a whole the process of researching and creating the gameplay logic, level design, animations and solving bugs has been a very time consuming but educational process.
